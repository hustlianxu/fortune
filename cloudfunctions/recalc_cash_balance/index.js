...
/** 判断交易类型是增加余额还是减少余额 */
function cashFlow(type, amount) {
  const amt = Number(amount) || 0;
  if (['transfer_in', 'sell', 'dividend', 'interest'].indexOf(type) >= 0) return amt;      // 收入
  if (['transfer_out', 'buy', 'ipo_win', 'fee', 'tax'].indexOf(type) >= 0) return -amt;     // 支出
...
exports.main = async (event) => {
  ...
  try {
    ...
    for (const t of allTxns) {
      netCashFlow += cashFlow(t.type, t.amount);
    }
